  ...
  if (bookJSON != null) {
    ...
    //console.log('x = '+x);
      ...
      for(let i = 0; i< x.length;i++) {
          ...
          <p> ${bookObj.title} </p>
          <p> ${bookObj.author} </p>
          <button type="button" class="remove-btnRet" myIndex ="${i}" >Remove</button>
          </div>
          `;
          const eachBookElement = parser.parseFromString(temp.innerHTML, 'text/html').body.firstChild;
          console.log('New Book elem= '+eachBookElement.innerHTML);   
          //console.log(newBookElement.innerHTML); 
          const btnRemove = eachBookElement.querySelector('.remove-btnRet');
          console.log('button remove inside wind= '+btnRemove.className);
          btnRemove.addEventListener('click', (index) => {           

            //console.log('Hello');
            JSON.parse(bookJSON).filter(() => {
              if(index === btnRemove.getAttribute('myIndex')){
                return false;
              }
              return true;
            });
            //if(e.target.getAttribute)
            //removeBook(e, eachBookElement);
            eachBookElement.remove();
            syncStorage();
          });
